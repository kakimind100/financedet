...
def prepare_data(df):
    ...
    # 종목 코드별로 최근 10일 데이터 확인
    for stock_code in df['Code'].unique():
        stock_data = df[df['Code'] == stock_code].tail(10)  # 최근 10일 데이터
        
        if len(stock_data) == 10:  # 최근 10일 데이터가 있는 경우
            X.append(stock_data[features].values.flatten())  # 10일의 피쳐를 1D 배열로 변환
    ...
    for stock_code in today_rise_stocks['Code'].unique():
        if stock_code in stock_codes_test:  # 테스트 데이터에 포함된 종목만 예측
            ...
            if not recent_data.empty:  # 데이터가 비어있지 않은 경우
                # 최근 10일 데이터를 사용하여 예측
                X_next = recent_data[features].values.flatten().reshape(1, -1)  # 10일 데이터로 2D 배열로 변환
