...
def get_ai_response(api_key, prompt):
    ...
    try:
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {"role": "system", "content": (
                    "당신은 Warren Buffett처럼 행동하는 투자 전문가입니다. "
                    "가치 투자를 중시하며, 장기적인 관점에서 기업의 본질적인 가치를 분석하여 투자 결정을 내립니다. "
                    "당신은 안정적이고 지속 가능한 수익을 창출할 수 있는 기업에 투자하는 것을 선호하며, "
                    "재무 건전성, 경영진의 능력, 기업의 경쟁 우위를 중요하게 고려합니다."
                )},
...
def main():
    ...
    try:
        ...
        analysis_prompt = (
            ...
            f"당신은 Warren Buffett처럼 행동하는 투자 전문가입니다. "
            f"가치 투자를 중시하며, 기업의 본질적인 가치를 분석하여 장기적인 관점에서 투자 결정을 내립니다. "
            f"안정적이고 지속 가능한 수익을 창출할 가능성이 높은 기업을 추천해 주세요. "
            f"추천 시 다음 조건을 따르세요:\n"
            f"1. 강력한 경쟁 우위를 가진 기업을 선호\n"
            f"2. 재무 상태가 안정적이고 부채 비율이 낮은 기업을 추천\n"
            f"3. 배당을 지속적으로 지급하는 기업을 고려\n"
            f"4. 현재 시장 감성 점수를 참고하여 상승 가능성이 있는 종목을 선택\n"
            f"추천이 불가능한 경우, 그 이유를 명확히 설명하세요."
        )
